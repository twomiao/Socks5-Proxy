...
class TcpServer
{
    ...
    /**
     * @var ?LoopInterface $eventLoopClass
     */
    public static ?LoopInterface $eventLoopClass = null;
    ...
    {
        if (PHP_OS !== 'Linux') {
            throw new \RuntimeException(sprintf("暂时不支持[ %s ]操作系统.", PHP_OS));
        }
    ...
    protected function listen()
    {
        ...
        if ($this->layerProtocol && !class_exists($this->layerProtocol)) {
            throw new \RuntimeException("[ {$this->layerProtocol} ] Application layer protocol not found.");
        }
        ...
        if (!$this->mainSocket) {
            ...
            if (!$this->mainSocket) {
                throw new \RuntimeException(sprintf("服务器监听端口 %s 失败, 错误原因:%s, 代码:%s.",
                        $this->getLocalPort(),
                        $error_msg,
                        $error_code
                    )
    ...
    {
        $signal_dispatch_handler = array($this, 'signalHandler');

        \pcntl_signal(\SIGHUP, $signal_dispatch_handler, false);
        \pcntl_signal(\SIGINT, $signal_dispatch_handler, false);
        \pcntl_signal(\SIGTERM, $signal_dispatch_handler, false);
    }
    ...
    protected function reinstallSignal()
    {
        ...
        static::$eventLoopClass->add(\SIGHUP, LoopInterface::EV_SIGNAL, $signal_dispatch_handler);
        static::$eventLoopClass->add(\SIGINT, LoopInterface::EV_SIGNAL, $signal_dispatch_handler);
        static::$eventLoopClass->add(\SIGTERM, LoopInterface::EV_SIGNAL, $signal_dispatch_handler);
    }
    ...
    public function stopWorkerAll($code = 0)
    {
...
EOF;
        ...
        switch ($command) {
            ...
            case 'stop':
                if (static::$smoothStop = ($mode === '-g')) {
                    $sig = \SIGHUP;
                    echo 'Swoman is gracefully stopping ...' . PHP_EOL;
                } else {
                    $sig = \SIGINT;
                    echo 'Swoman is stopping ...' . PHP_EOL;
                }
    ...
    protected function run()
    {
        ...
        static::$eventLoopClass = new Swoole();
        $this->resumeAccept();
        $this->reinstallSignal();
        Timer::init(static::$eventLoopClass);
        ...
        try {
            ...
            // TODO::
            echo 'Trigger error.' . PHP_EOL;
        } catch (\Error $e) {
            // TODO::
            echo 'Trigger error.' . PHP_EOL;
        }

        static::$eventLoopClass->loop();
    }
    ...
    protected function saveMasterPid()
    {
        ...
        if (false === \file_put_contents(static::$pidFile, static::$masterPid)) {
            throw new \RuntimeException('主进程文件保存失败:' . static::$pidFile);
        }
    }
    ...
    {
        if (static::$eventLoopClass && $this->mainSocket && $this->pauseAccept) {
            static::$eventLoopClass->add($this->mainSocket, LoopInterface::EV_READ, array($this, 'acceptTcpConnection'));
    ...
    {
        if (static::$eventLoopClass && $this->mainSocket && !$this->pauseAccept) {
            static::$eventLoopClass->del($this->mainSocket, LoopInterface::EV_READ);
    ...
    public function acceptTcpConnection($socket)
    {
        ...
        if ($this->onConnect) {
            try {
                ...
                exit('Connection event triggers an exception.');
            } catch (\Error $e) {
                exit('Connection event triggers an exception.');
            }
        }
